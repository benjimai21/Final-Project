...
public class ConnectivityBox : MonoBehaviour {
   ...
   // public GameObject m_RightController;
    ...
    void Start () {
        ...
        if (m_LeftController)
            ...
            print("ERROR: Missing left controller reference in ConnectivityBox!");

        if (m_RightController)
            m_RightTrackedContr = m_RightController.GetComponent<SteamVR_TrackedController>();
        else
            print("ERROR: Missing right controller reference in ConnectivityBox!");
    ...
    {
        if (m_controlManager.GetCurrentControlMode() == ControlModeManager.CONTROL_MODE.CONNECTIVITY_BOX)
            ShowConnections();
    }
    ...
    private void ShowConnections()
    {
        if (m_lowerSphere && m_upperSphere)
        {
            ...
            m_LineModelRenderer.ShowConnectivity(lower, upper);
        }
        else
        {
            print("ERROR: bounding spheres not assigned on ConnectivityBox");
        }
    }
    ...
    void Update()
    {
        if (m_controlManager.GetCurrentControlMode() == ControlModeManager.CONTROL_MODE.CONNECTIVITY_BOX && !m_boxVisible)
        {
            for (int i = 0; i < transform.childCount; i++)
                transform.GetChild(i).gameObject.SetActive(true);
            m_boxVisible = true;
        }
        else if (m_controlManager.GetCurrentControlMode() != ControlModeManager.CONTROL_MODE.CONNECTIVITY_BOX && m_boxVisible)
        {
            for (int i = 0; i < transform.childCount; i++)
